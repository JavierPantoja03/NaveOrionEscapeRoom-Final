...
@Component({
  selector: 'app-actividad-2',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './actividad-2.html',
  styleUrl: './actividad-2.css'
})
export class Actividad2 implements AfterViewInit {

  showMessage: boolean = false;
  isCorrect: boolean = false;
  isFullScreen: boolean = false; 
  showSolutionContent: boolean = true;
  showActivity3Button: boolean = false;
  ...
  goBack() {
    this.router.navigate(['/actividad-1']); 
  }
  ...
  checkAnswer(answer: string) {
    ...
    // Lógica: La respuesta 'A' (sin importar la etiqueta del botón) es la correcta.
    if (answer === 'A') { 
      this.isCorrect = true;
  ...
  showComputationalExplanation() {
    ...
    // Muestra el botón para ir a Actividad 3
    this.showActivity3Button = true;
  }

  closeMessage() {
    this.showMessage = false;
    this.showSolutionContent = true;
    this.showActivity3Button = false; 
  }
  
  goToActivity3() {
    this.router.navigate(['/actividad-3']); // Navega a la siguiente actividad
  }
